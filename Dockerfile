...
COPY . .
WORKDIR "/src/."
RUN dotnet build "./ERPAPP.csproj" -c $BUILD_CONFIGURATION -o /app/build

# 此階段可用來發佈要複製到最終階段的服務專案
FROM build AS publish
ARG BUILD_CONFIGURATION=Release
RUN dotnet publish "./ERPAPP.csproj" -c $BUILD_CONFIGURATION -o /app/publish /p:UseAppHost=false
